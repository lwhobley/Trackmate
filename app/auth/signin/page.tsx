...
function SignInForm() {
  ...
  return (
    <div className="glass rounded-3xl p-7">
      {error && (
        <div className="mb-4 text-red-600 text-sm bg-red-50 border border-red-100 rounded-2xl p-3">{error}</div>
      )}
      ...
      <div className="relative z-10 w-full max-w-sm fade-up">
        ...
        <Suspense fallback={
          <div className="glass rounded-3xl p-7 space-y-4">
            {[1,2].map(i => <div key={i} className="h-11 rounded-xl shimmer" style={{background:'rgba(108,99,255,0.06)'}} />)}
            <div className="h-11 rounded-full shimmer" style={{background:'rgba(255,75,0,0.1)'}} />
          </div>
        }>
          <SignInForm />
        </Suspense>
